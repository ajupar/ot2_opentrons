...
class Species:
    ...
    def get_current_source_well(self, volume_to_transfer: float):
        ''' get the first source well with enough fluid '''
        for well in self.source_wells:
            if well.can_be_transferred_from(volume_to_transfer):
                return well
        raise Exception("could not find source well with enough fluid")
...
class Combination:
    ...
    def __init__(self, specie, target_well):
        self.specie = specie
        self.target_well = target_well

    def pipette_combination(self, pipette: protocol_api.InstrumentContext, total_volume: float, change_pipettes: bool):
        ''' Pipette the species contained in this combination '''
        ...
        for spec in self.specie:
            pipette.transfer(total_volume / len(self.specie), )


##########################
# This part can be modified by the user:
# script starting values
SOURCE_WELLS_INITIAL_VOLUME_UL = 150.0 # modify this based on initial volume; affects how source well is changed
...
'''
TOUCH_SIDE_AFTER_DISPENSE = True
####################
...
def initialize_combinations_list():
...
combinations_list = initialize_combinations_list()
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    tiprack_300ul = protocol.load_labware("opentrons_96_tiprack_300ul", 4)

    master_plate1 = protocol.load_labware("biorad_96_wellplate_200ul_pcr", 5)  # pitää ilmeisesti muuttaa Sarstedtin custom-levyksi
    master_plate2 = protocol.load_labware("biorad_96_wellplate_200ul_pcr", 6)
    reservoir_plate = protocol.load_labware("biorad_96_wellplate_200ul_pcr", 7)
    ...
    right = protocol.load_instrument("p300_single_gen2", "right", [tiprack_300ul])
